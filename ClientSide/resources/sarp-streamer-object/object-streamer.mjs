...
async function loadModel(model) {
  return new Promise((resolve) => {
    if (!natives.isModelValid(model)) return resolve(false);

    if (natives.hasModelLoaded(model)) return resolve(true);
...
class ObjectStreamer {
  ...
  async addObject(
    ...
    loadModel(model).then(() => {
      if (entityType == 2) {
        let handle = natives.createObject(
          natives.getHashKey(model),
          pos.x,
          pos.y,
          pos.z,
          false,
        ...
        this.setOnGround(+entityId, onGround);
  ...
  async restoreObject(entityId, entityType) {
    if (this.objects.hasOwnProperty(entityId)) {
      ...
      loadModel(obj.model).then(() => {
        if (entityType == 2) {
          this.objects[entityId].handle = natives.createObject(
            natives.getHashKey(obj.model),
            obj.position.x,
            obj.position.y,
            obj.position.z,
            false,
          ...
          this.setOnGround(+entityId, obj.onground);
          //this.setVisible( obj, obj.visible );
          //this.setOnFire(+entityId, obj.onFire);
          this.setFrozen(+entityId, obj.frozen, entityType);
          //this.setLightColor(+entityId, obj.lightColor);
        } else if (entityType == 4) {
          this.objects[entityId].handle = natives.createPed(
            26,
            natives.getHashKey(obj.model),
            obj.position.x,
            obj.position.y,
  ...
  removeObject(entityId, entityType) {
    if (this.objects.hasOwnProperty(entityId)) {
      if (entityType == 2) {
        ...
        natives.deletePed(this.objects[entityId].handle);
      }
  ...
  setTextureVariation(entityId, textureVariation = null) {
    if (this.objects.hasOwnProperty(entityId)) {
      natives.setObjectTextureVariation(
        this.objects[entityId].handle,
        textureVariation
      );
  ...
  setOnGround(entityId, onground) {
    if (this.objects.hasOwnProperty(entityId) && onground !== null) {
      natives.placeObjectOnGroundProperly(this.objects[entityId].handle);
      this.objects[entityId].onground = !!onground;
    }
  }
  ...
  setOnFire(entityId, onFire = null) {
    if (this.objects.hasOwnProperty(entityId) && onFire !== null) {
      if (!!onFire) {
        this.objects[entityId].fireHandle = natives.startScriptFire(
          this.objects[entityId].position.x,
          this.objects[entityId].position.y,
          this.objects[entityId].position.z,
          1,
          true
        );
      } else {
  ...
  setFrozen(entityId, frozen, entityType) {
    if (this.objects.hasOwnProperty(entityId) && frozen !== null) {
      if (this.objects[entityId].entityType == 4) {
        natives.setEntityInvincible(this.objects[entityId].handle, true);
        natives.taskSetBlockingOfNonTemporaryEvents(
          this.objects[entityId].handle,
          true
        );
      }
